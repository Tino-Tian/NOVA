...
/// Marker used to identify NOVA-injected blocks in shell profile files.
#[cfg(not(target_os = "windows"))]
const APP_PATH_MARKER: &str = "# Added by NOVA";

/// Strip all `# Added by NOVA\nexport PATH=...` blocks from a shell profile.
///
...
/// 3. Strip all previous NOVA blocks from that profile to prevent
...
pub fn inject_path(cli_path: &str) -> Result<String, String> {
    ...
    // Strip all historical NOVA blocks. This fixes two bugs at once:
